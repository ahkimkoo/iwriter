    ...
    if(token){
        ...
        if(hkex_market_data){
            ...
            if(hsq.length>0){
                ...
                message += `\n恒生指数${hs_change_sytax}%，报${hs_price}点；`;
            }
            if(gqq.length>0){
                let gq = gqq[0];
                ...
                message += `\n国企指数${gq_change_sytax}%，报${gq_price}点；`;
            }
        }
    }
}
...
if(hgt_doc){
    ...
    try{
        ...
        if(northbound12){
            ...
            message += `\n沪股通净流${hgt_change_sytax}${Math.abs(hgt_change)}亿元，当日余额${hgt_amount.toFixed(2)}亿元；`;
        }
    }catch(e){
...
if(sgt_doc){
    ...
    try{
        ...
        if(northbound22){
            ...
            message += `\n深股通净流${sgt_change_sytax}${Math.abs(sgt_change)}亿元，当日余额${sgt_amount.toFixed(2)}亿元。`;
        }
    }catch(e){
...
if(date){
    let now = new Date();
    let mq_message = `☑港股午间收盘数据（${dateFormat(now,'M月d日 hh:mm')}）\n\n${message}`; 
