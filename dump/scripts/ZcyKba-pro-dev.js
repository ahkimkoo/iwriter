    ...
    return new Promise(resolve=>{
        MongoClient.connect('mongodb://mongodb.feheadline.net:27017', function(err, client) {
            if(err)resolve(null);
                    ...
                    let db = client.db('feheadline');
                    let collection = db.collection('user_stock_bet_match');
                    collection.find({"create_time":{"$lte":new Date('2019-02-11 12:00:00'),"$gte":new Date('2019-01-30 12:00:00')}}).toArray(async (err,docs)=>{
                        client.close();
                        if(!err && docs.length>0){
                            resolve(docs);
                        } else {
...
const updatec = async (bet_result,user_id)=>{
        return new Promise(resolve=>{
        let connection = mysql.createConnection({
          'host'     : '10.10.119.133',
          'user'     : 'fes',
        ...
        connection.connect();
        let sql =  `update fes_activity_submit set valid =1`;
        connection.query(sql, async function (error, result, fields) {
    ...
    return new Promise(resolve=>{
        MongoClient.connect('mongodb://mongodb.feheadline.net:27017', function(err, client) {
            if(err)resolve(null);
                    ...
                    let db = client.db('feheadline');
                    let collection = db.collection('stock_bet_match');
                    collection.find({}).sort(["create_time",-1]).limit(10).toArray(async (err,docs)=>{
                        client.close();
                        if(!err && docs.length>0){
                            resolve(docs);
                        } else {
...
const updateUserResultAfter = async ()=>{
    let MongoClient = mongodb.MongoClient;
    return new Promise(resolve=>{
        MongoClient.connect('mongodb://mongodb.feheadline.net:27017', function(err, client) {
            if(err)resolve(null);
                ...
                let db = client.db('feheadline');
                let collection = db.collection('user_stock_bet_match');
                collection.updateMany({"_id":new mongodb.ObjectID("5c934ba60d380900e7cb1cd9")},{"$set":{"match_result":"2.690","bet_result":0}},null, (err,docs)=>{
    ...
    return new Promise(resolve=>{
        MongoClient.connect('mongodb://mongodb.feheadline.net:27017', function(err, client) {
            if(err)resolve(null);
                    ...
                    let db = client.db('feheadline');
                    let collection = db.collection('user_stock_bet_match');
                    collection.find({"user_id":5254423}).sort(["create_time",-1]).limit(3).toArray(async (err,docs)=>{
                        client.close();
                        if(!err && docs.length>0){
                            resolve(docs);
                        } else {
...
const updateMysqlorigin = async (bet_result,user_id)=>{
        return new Promise(resolve=>{
        let connection = mysql.createConnection({
          'host'     : '10.10.119.133',
          'user'     : 'news',
...
(async ()=>{
	...
	process.exit(0);
        ...
        connection.connect();
        let sql =  "update fe_news_video set create_time =now(),pub_time=now() where id =4009488;";
        connection.query(sql, async function (error, result, fields) {
                    connection.end();
                    resolve(result);
...
const updateorigin = async ()=>{
    let result = await updatec();
    return result;
}
//return getOneMatch();
return updateMysqlorigin()
