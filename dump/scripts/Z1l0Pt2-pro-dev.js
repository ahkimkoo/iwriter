...
/*embed script begin*/
    ...
    'caimi_web' : '网页版微信',
    'fes_app': 'APP微信',
    'fes_app_alipay': 'APP支付宝',
    'fes_wxapp': '微信小程序'
}

const getPlatform = (code) => {
    let default_ret = '小程序';
    if(code){
        if(platform_mapping.hasOwnProperty(code))return platform_mapping[code];
        else code;
...
(async ()=>{
	...
	process.exit(0);
        ...
        let sql = `SELECT 
            ...
            wx_order_history.platform AS platform, 
            ...
            IF(ISNULL(fes_user_subscribe_buy.salesman),
                ...
                (SELECT 
                    ...
                    FROM
                        fes_user
                    WHERE
                        fe_user_id = fes_user_subscribe_buy.salesman)) AS salesman,
            fes_user_subscribe_buy.buy_price AS require_price,
            wx_order_history.total_fee AS total_fee,
            wx_order_history.create_time AS create_time
        FROM
        ...
        WHERE
            fes_user_subscribe_buy.valid = 1
                ...
                AND wx_order_history.create_time > FROM_UNIXTIME(${parseInt(cursor.getTime()/1000)})
                AND wx_order_history.create_time <= FROM_UNIXTIME(${parseInt(now.getTime()/1000)})
        ...
        connection.query(sql, async function (error, results, fields) {
          if (error){
              ...
              let tidy_order = results.reduce((dlist, ret)=>{
                  ...
                  if(idx>=0){
                      ...
                      dlist.push({
                          ...
                          'platform': getPlatform(ret['platform']),
                          'orders':[{
                          'industry_name' : ret['industry_name'] ,
                          'require_price' : ret['require_price'],
                          'start_time' : ret['start_time'],
              ...
              let tidy_list = [['昵称','手机','时间','费用','行业','代理商','推荐人','平台']];
              
              for(let ret of tidy_order){
                  let mq_message = `☑财秘订单：\n${ret['buyer'].replace(/(.)./g,'$1░')}（${ret['buyer_phone'].replace(/(\d{3})\d{4}(\d+)/,'$1****$2')}）于${dateFormat(ret['create_time'],'hh:mm')}在${ret['platform']}付费${ret['total_fee']}元购买了${ret['orders'].length}份行业情报：${ret['orders'].map(x=>{return x['industry_name']}).join('；')}。`;
                  if(ret['agency']){
                      mq_message += `\n此单代理商：${ret['agency']} 。`;
                  }
                  if(ret['salesman']){
                  ...
                  tidy_list.push([ret['buyer'],ret['buyer_phone'],dateFormat(ret['create_time'],'hh:mm:ss'),ret['total_fee'],ret['orders'].map(x=>{return x['industry_name']}).join('；'),ret['agency'],ret['salesman'],ret['platform']])
              }
