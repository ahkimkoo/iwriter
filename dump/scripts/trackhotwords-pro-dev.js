...
/*embed script begin*/
        ...
        // {'keyword':'易会满发言', 'only_title':true},
        // {'keyword':'指数暴跌', 'only_title':true},
    ]
    ...
    for(let section in warehouse){
        if(warehouse.hasOwnProperty(section)){
            if(warehouse[section]['news'].length>0){
                ...
                html += '</ul>';
                let mq_message = `☑${section}：\n\n${warehouse[section]['news'].slice(0,8).map(x=>{return x['title']}).join('\n\n')}\n\n详见：http://dwz.cn/Z8p8APha`;
                if(CALLER!='coder')await sendMQ('amqp://fenews:fenews@rabbit-crawler.feheadline.net:5672/fenews','iw_ex',{'to':'舆情监控','message':mq_message});
            }
