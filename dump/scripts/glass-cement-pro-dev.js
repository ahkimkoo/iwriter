...
const getItemsPrice = async(url, name_filter, date)=>{
    let data_array = await getTableData(url, '.lp-table', [{'index':4,'values':[`${date}`]}, {'index':0,'values': name_filter}]);
    if(data_array){
...
var entities = [
    ['http://cement.100ppi.com/price/', ['安徽地区','江苏地区','山东地区','福建地区','浙江地区','上海市场'], '水泥（市场价：元/吨）'],
