...
//                filename: 'logs/rcmd-'+name+'-'+(arguments.length>=3?id:process.pid)+'.ac.log',
                stream: fs.createWriteStream('logs/iwriter-'+name+'-'+(arguments.length>=3?id:process.pid)+'.ac.log', { flags: 'a' }),
    ...
    if(arguments.length>=4){
        ...
        if(mode>=110)logger.add(
            winston_daily_rotate,
            {
                ...
                stream: fs.createWriteStream('logs/iwriter-'+name+'-'+(arguments.length>=3?id:process.pid)+'.log', { flags: 'a' }),
                datePattern: '.yyyy-MM-dd',
                level: level.toLowerCase(),
                json: false,
